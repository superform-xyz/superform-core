...
contract MDMVDMulti021120NoNativeSlippageAMB23 is ProtocolActions {
    ...
    function test_scenario(uint128 amountOne_, uint128 amountTwo_, uint128 amountThree_) public {
        ...
        amountOne_ = uint128(bound(amountOne_, 2 * 10 ** 18, TOTAL_SUPPLY_ETH / 6));
        amountTwo_ = uint128(bound(amountTwo_, 2 * 10 ** 18, TOTAL_SUPPLY_ETH / 6));
        amountThree_ = uint128(bound(amountThree_, 2 * 10 ** 18, TOTAL_SUPPLY_ETH / 6));
