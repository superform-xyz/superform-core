...
contract MDSVD4626RevertTimelockedNoTokenInputNoSlippageAMB23 is ProtocolActions {
    ...
    function test_scenario(uint128 amountOne_, uint128 amountTwo_, uint128 amountThree_) public {
        amountOne_ = uint128(bound(amountOne_, 1 * 10 ** 18, TOTAL_SUPPLY_WETH / 3));
        amountTwo_ = uint128(bound(amountTwo_, 1 * 10 ** 18, TOTAL_SUPPLY_WETH / 3));
        amountThree_ = uint128(bound(amountThree_, 1 * 10 ** 18, TOTAL_SUPPLY_WETH / 3));
