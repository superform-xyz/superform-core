        ...
        for (uint256 act = 0; act < actions.length; ++act) {
            ...
            if (act == 1) {
                for (uint256 i = 0; i < DST_CHAINS.length; ++i) {
                    ...
                    /// superform)
                    amountOneWithdraw_ = uint128(bound(amountOneWithdraw_, 1e6, (superPositions[0] / 3) + 1));
                    amountTwoWithdraw_ = uint128(bound(amountTwoWithdraw_, 1e6, (superPositions[1] / 3) + 1));
                    amountThreeWithdraw_ = uint128(bound(amountThreeWithdraw_, 1e6, (superPositions[2] / 3) + 1));
