...
describe("Router Patch Unit Testing", () => {
  ...
  async function buildWithdrawCall(vaultId, sharesAmount, targetChainId) {
    ...
    const stateReq = [
      targetChainId,
      [sharesAmount], /// NOTE: amount is irrelevant on processWithdraw, err?
      [vaultId],
  ...
  it("2: overwithdraw from vault id 421612 (FAILS, AS SHOULD)", async function () {
    try {
      ...
      let balanceOf421612 = await router.balanceOf(user.address, 421612); /// Router returns zero shares here?
      let balanceOf421615 = await router.balanceOf(user.address, 421615);
