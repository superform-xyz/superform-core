...
interface ICoreStateRegistryHelper {
    /// @dev reads the payload from the core state registry and decodes it
    /// @param payloadId_ is the unique identifier of the payload received in dst core state registry
    ...
    /// @return data is the encoded data passed from the source chain. Use the below function to decode it.
    function decodePayloadInfo(
        uint256 payloadId_
    ) external view returns (uint8 txType, uint8 callbackType, address srcSender, uint64 srcChainId, bytes memory data);
